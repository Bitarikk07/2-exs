        ...
        <div class="flex w-full justify-between items-center p-[0.5rem] bg-[#F8F8F8] rounded-xl">
            <div class="flex items-center gap-[1rem]">
                <div class="flex flex-col items-start gap-[0.25rem]">
                    <p class="text-[#999999] {{ Route::is('two') ? 'text-[0.65rem]' : 'text-[0.875rem]' }} ">
                        Highest bid</p>
                    <h2 class="  {{ Route::is('two') ? 'text-[0.8rem]' : 'text-[1.25rem]' }} text-[#101010] font-bold">
                        {{ $list->currency }} {{ $list->price }}</h2>
                </div>

            </div>
            <button
                class="py-[0.25rem]  {{ Route::is('two') ? 'text-[0.45rem]' : 'text-[0.75rem]' }} text-[#151517] font-semibold px-[0.5rem] bg-[#DCF06B] rounded-[2rem]">
                {{ $list->end }}
            </button>
        </div>
    </div>
